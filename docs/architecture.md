...
2. **On-device language model.** WebLLM runs Qwen3.5 4B on WebGPU by default, chosen by the tier-matched benchmark in [benchmarks.md](benchmarks.md) and small enough for an 8 GB laptop; MiniCPM5 2B takes over automatically only where 4B is over the device's GPU budget, and Qwen3.5 9B is a manual pick. MiniCPM5 2B has no upstream WebLLM build, so this project quantized and compiled it and hosts it at [ozhyhinas/MiniCPM5-2B-q4f16_1-MLC](https://huggingface.co/ozhyhinas/MiniCPM5-2B-q4f16_1-MLC); `app.js` merges that record into WebLLM's prebuilt list (`PUBLISHED`). The default loads directly once the specialist is up; the GPU budget and browser storage quota decide whether it is picked automatically or by hand. Prompts are kept terse because prefill dominates short generations on laptop GPUs. On an Apple GPU with WGSL subgroups behind Chromium 152 or newer, the graph catalog's patched WebLLM runtime (family `qwen3-webllm`) replaces stock decoding with device-resident greedy argmax and batched command encoding, decoding noticeably faster at temperature 0; the catalog carries tuned model libs for all three (and for the Qwen3 sizes they replaced). Anywhere else, or if that runtime fails to load, it falls back to stock WebLLM.
