...
flowchart LR
    ...
    C -- checks fail --> D[On-device LLM<br/>WebLLM, Qwen3.5 4B default · 9B, MiniCPM5 2B]
    D -- checks pass --> Z
...
2. **On-device language model.** WebLLM runs Qwen3.5 4B on WebGPU by default, chosen by the tier-matched benchmark in [benchmarks.md](benchmarks.md) and small enough for an 8 GB laptop; MiniCPM5 2B takes over automatically only where 4B is over the device's GPU budget, and Qwen3.5 9B is a manual pick. MiniCPM5 2B has no upstream WebLLM build, so this project quantized and compiled it and hosts it at [ozhyhinas/MiniCPM5-2B-q4f16_1-MLC](https://huggingface.co/ozhyhinas/MiniCPM5-2B-q4f16_1-MLC); `app.js` merges that record into WebLLM's prebuilt list (`PUBLISHED`). The default's download starts once the specialist's weights have arrived and overlaps its compile, so the two never fight over the connection; the GPU budget and browser storage quota decide whether it is picked automatically or by hand. Prompts are kept terse because prefill dominates short generations on laptop GPUs. On an Apple GPU with WGSL subgroups behind Chromium 152 or newer, the graph catalog's patched WebLLM runtime (family `qwen3-webllm`) replaces stock decoding with device-resident greedy argmax and batched command encoding, decoding noticeably faster at temperature 0; the catalog carries tuned model libs for all three (and for the Qwen3 sizes they replaced). Anywhere else, or if that runtime fails to load, it falls back to stock WebLLM.
...
- WebLLM does not run its stock decoding path when the graph catalog's runtime applies: `qwen3-webllm.js` picks one of three patched WebLLM 0.2.84 bundles (vendored under `vendor/webllm/`, family `qwen3-webllm`) plus a per-model TVM model lib streamed straight from the catalog dataset; the model weights are the stock `mlc-ai` q4f16_1 shards, shared with the stock path, so only the runtime bundle and the model lib differ. The fast path (device-resident greedy argmax, K-token decode bursts, batched command encoding with periodic flushes, optional lookahead, and on the Qwen3.5 rungs prompt-lookup drafting that verifies copied prose in one multi-token forward) only engages at `temperature: 0` with no `logprobs`, which is also why `pipeline.js` now decodes greedily. The target probe requires an Apple GPU adapter exposing the `subgroups` feature on Chromium 152 or newer; a failed probe or a failed engine load falls back to stock WebLLM and is remembered per model for 7 days, so a transient failure does not pin stock forever. `?webllm=stock` and `?webllm=catalog[:variant]` override the pick for benchmarking.
- Weights are stored through WebLLM's OPFS cache backend rather than the Cache API: every shipped model has a single-tensor shard of 128–485 MB, and at least one Chromium build refuses Cache API bodies above roughly 250 MB. A model whose shards are already complete in the Cache API keeps using it, so nothing is downloaded twice.
- All conversion logic lives in `pipeline.js`, with no DOM access. The UI, the Tab API and mesh jobs call the same functions.

## Files

```
public/app.js          UI wiring: drawers, model picker, model load, Tab API client
