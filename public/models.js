...
// on WebGPU int4 with identical outputs; Texo is overhead-bound (stays CPU
// fp32); int4 on CPU is 10x slower than int8, so int4 is GPU-only.
export function pickRuntime(model) {
  if (model === "texo") return { dtype: "fp32" };
...
export function loadModel(key, onProgress) {
  loaders[key] ??= (async () => {
    ...
    try {
      ...
      if (cfg.device === "webgpu") rememberRuntime(key, true);
      runtimeUsed[key] = cfg; loaded[key] = true;
    } catch (err) {
      if (cfg.device === "webgpu") { rememberRuntime(key, false); console.warn(`${key}: WebGPU failed, CPU on next load`, err); }
      delete loaders[key];
      throw err;
    } finally {
